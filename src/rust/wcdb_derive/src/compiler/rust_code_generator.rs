...
impl RustCodeGenerator {
    pub(crate) fn generate(&self, table: &WCDBTable) -> syn::Result<proc_macro2::TokenStream> {
        ...
        Ok(quote::quote! {
            pub static #binding_ident: once_cell::sync::Lazy<wcdb::orm::binding::Binding> = once_cell::sync::Lazy::new(|| {
                wcdb::orm::binding::Binding::new()
            });
            ...
            pub struct #db_table_ident {
                #(
                    pub #field_ident_vec: *const wcdb::orm::field::Field<#table_ident>
                ),*
            }
                    ...
                    unsafe {
                        #(
                            if !self.#field_ident_vec.is_null() {
                                Box::from_raw(self.#field_ident_vec as *mut wcdb::orm::field::Field<#table_ident>);
                            }
                        )*
                    }
                }
...
impl RustCodeGenerator {
    fn generate_fields(
        ...
        for column_info in &self.all_column_info {
            ...
            fields_token_stream.extend(quote! {
                pub fn #name_ident() -> &'static wcdb::orm::field::Field<#table_ident> {
        ...
        Ok(quote! {
            impl #db_table_ident {
                pub fn all_fields() -> Vec<&'static wcdb::orm::field::Field<#table_ident>> {
                    unsafe { vec![
                        #(
    ...
    fn generate_columns(&self, binding_ident: &Ident) -> syn::Result<proc_macro2::TokenStream> {
        ...
        for column_info in &self.all_column_info {
            ...
            token_stream.extend(quote! {
                let field = Box::new(wcdb::orm::field::Field::new(
            ...
            token_stream.extend(quote::quote! {
                let create_index = wcdb::winq::statement_create_index::StatementCreateIndex::new();
    ...
    fn generate_binding_fields(
        ...
        Ok(quote::quote! {
            fn all_binding_fields(&self) -> Vec<&wcdb::orm::field::Field<#table_ident>> {
                unsafe { vec![
                    #(
    ...
    fn generate_base_binding(
        ...
        Ok(quote::quote! {
            fn base_binding(&self) -> &wcdb::orm::binding::Binding {
                &*#binding_ident
            }
        })
    }
    ...
    fn generate_extract_object(
        ...
        for column_info in all_column_info_vec {
            ...
            if field_orm_info.nullable {
                extract_token_stream_vec.push(quote! {
                    #column_index => {
                        if prepared_statement.get_column_type(index) != wcdb::winq::column_type::ColumnType::Null {
        ...
        Ok(quote::quote! {
             fn extract_object(
                &self,
                fields: &Vec<&wcdb::orm::field::Field<#table_ident>>,
                prepared_statement: &wcdb::core::prepared_statement::PreparedStatement,
            ) -> #table_ident {
    ...
    fn generate_bind_object(&self, table_ident: &&Ident) -> syn::Result<proc_macro2::TokenStream> {
        ...
        Ok(quote::quote! {
             fn bind_field(
                &self,
                object: &#table_ident,
                field: &wcdb::orm::field::Field<#table_ident>,
                index: usize,
                prepared_statement: &std::sync::Arc<wcdb::core::prepared_statement::PreparedStatement>,
            ) {
                match field.get_field_id() {
                    #(
                    ...
                    _ => panic!("Invalid id {} of field {} in {}",
                        field.get_field_id(),
                        wcdb::winq::identifier::IdentifierTrait::get_description(field),
